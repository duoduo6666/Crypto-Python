...
class SHA1s:
    ...
    def __calculate(self,data:bytes,h:bytes) -> bytes:
        ...
        for ib in [data[i:i + 64] for i in range(0,len(data),64)]:
            ...
            for i in [ib[i:i + 4] for i in range(0,len(ib),4)]:
                W.append(int.from_bytes(i,byteorder))
            for t in range(16,80):
                W.append(self.__ROTL(W[t-3] ^ W[t-8] ^ W[t-14] ^ W[t-16],1))
            
            # 计算
            a,b,c,d,e = h[0],h[1],h[2],h[3],h[4]

            for i in range(0,20):
                a,b,c,d,e = (self.__ROTL(a,5) + ((b & c) | ((~ b) & d)) + e + 0x5a827999 + W[i]) & 0xffffffff,a,self.__ROTL(b,30),c,d
            for i in range(20,40):
                a,b,c,d,e = (self.__ROTL(a,5) + (b ^ c ^ d) + e + 0x6ed9eba1 + W[i]) & 0xffffffff,a,self.__ROTL(b,30),c,d
            for i in range(40,60):
                a,b,c,d,e = (self.__ROTL(a,5) + ((b & c) ^ (b & d) ^ (c & d)) + e + 0x8f1bbcdc + W[i]) & 0xffffffff,a,self.__ROTL(b,30),c,d
            for i in range(60,80):
                a,b,c,d,e = (self.__ROTL(a,5) + (b ^ c ^ d) + e + 0xca62c1d6 + W[i]) & 0xffffffff,a,self.__ROTL(b,30),c,d
    ...
    def __init__(self,data:bytes=b'') -> None:
        ...
        self.__ROTL = lambda x,n:((x << n) | (x >> (32 - n))) & 0xffffffff
